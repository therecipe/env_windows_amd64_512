...
func main() {
	pPath := strings.Replace(filepath.Join("C:\\", "Users", "Public", "env_windows_amd64", "5.12.0", "mingw73_64"), string(filepath.Separator), "\\", -1)
	if len(os.Args) >= 2 {
		pPath = os.Args[1]
	}
	if !strings.Contains(pPath, "5.12.0") {
		pPath = strings.Replace(filepath.Join(pPath, "5.12.0", "mingw73_64"), string(filepath.Separator), "\\", -1)
	}
	...
	for _, fn := range []string{
		...
		fn = filepath.Join("./5.12.0/mingw73_64/", fn)
	...
	fn := filepath.Join("./5.12.0/mingw73_64/bin/qtenv2.bat")
	data, err := ioutil.ReadFile(fn)
	if err != nil {
		println("couldn't find", fn)
		return
